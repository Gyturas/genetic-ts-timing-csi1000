...
def main():
    ...
    C.END = str(pd.read_csv(os.path.join(根, "data_cache", "idx_sh000852.csv"),
    ...
    名册 = pd.read_csv(os.path.join(根, "数据", "名册.csv"))
    元老 = set(名册[名册.kind.isin(["elder", "argarch"])]["编号"])
    存 = pd.read_csv(os.path.join(根, "结果_26算子", "因子逐日信号.csv.gz"), parse_dates=["date"])
