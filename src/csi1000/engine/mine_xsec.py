# -*- coding: utf-8 -*-
"""截面叶子挖矿:开盘口径 + TERMINALS 扩容(9→16,含截面结构叶子)。

基于 mine_open_horizons,叶子注入见 xsec_leaves.注入;其余制度不变。
...
def main():
    ...
    E.输出目录 = os.path.join(paths.根, "results", f"mine_xsec{a.tag}", f"h{a.h}_s{a.seed}")
    E.GA参数["seed"] = a.seed
    E.安装GA()
    n0 = len(gops.OPS)
    for k in 正典外算子:
    ...
    数据 = E.load_all()
    from csi1000.engine.xsec_leaves import 注入, 新叶子
    注入(数据["classes"]["A股宽基"]["panels"])
    gops.TERMINALS = gexpr.TERMINALS = tuple(C.TERMINALS) + 新叶子
    import csi1000.ga_alpha.expr as _e
    print(f"[截面挖矿] TERMINALS {len(C.TERMINALS)}→{len(gops.TERMINALS)}: +{新叶子}")
