...
def main():
    末 = pd.read_csv(os.path.join(paths.行情缓存, "idx_sh000852.csv"),
                    usecols=["date"])["date"].max()
    print(f"行情末日 {末}(若不是最新收盘日,请先更新行情再跑)")

    from csi1000.engine.final_signal import main as 生成
    生成()

    π = pd.read_csv(os.path.join(paths.存档, "映射分位.csv"),
                    parse_dates=["date"]).set_index("date")
    S = pd.read_csv(os.path.join(paths.存档, "分资产逐日.csv"), index_col=0,
