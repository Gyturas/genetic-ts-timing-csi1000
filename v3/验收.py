...
检查项:
  ...
  ⑦ 隔离运行          整仓复制到临时目录、剥掉 .git,在那里跑:
      ...
      重放一致性     用存档按正典权重重算,须与挖矿当场算的逐日仓位相同(差<1e-12)
      换权重可用     重算权重 --方案 等权   须跑通
...
def 静态检查() -> None:
    ...
    必需 = {
        ...
        "引擎": ["__init__.py", "指数日频引擎.py", "重算权重.py", "策略.py", "路径.py",
    ...
    if os.path.exists(pkl):
        ...
        with open(pkl, "rb") as f:
            查("本地面板缓存与当前 END 一致", pickle.load(f).get("built_end") == C.END,
               "缓存是别的 END 建的;load_all 会自动重建,但别把它提交上去")
...
def 隔离运行() -> None:
    ...
    # 重放一致性:用存档按"正典"权重重算,必须与挖矿当场算出的逐日仓位相同。
    # 对不上就说明存档漏了信息或结算被改坏了 —— 换权重的实验全部不可信。
    查("挖矿产出 因子逐日信号.csv.gz",
       os.path.exists(os.path.join(仓, "结果_挖矿", "因子逐日信号.csv.gz")))
    rr = 跑("引擎.重算权重", "--方案", "正典", "--输出", "结果_正典复算")
    查("重算权重跑通", rr.returncode == 0, rr.stderr[-300:])
    try:
        import pandas as pd
        差 = 0.0
        for f in ("资产暴露.csv", "组合逐日.csv", "分资产逐日.csv"):
            x = pd.read_csv(os.path.join(仓, "结果_挖矿", f), index_col=0, parse_dates=True)
            y = pd.read_csv(os.path.join(仓, "结果_正典复算", f), index_col=0, parse_dates=True)
            i = x.index.intersection(y.index)
            差 = max(差, float((x.loc[i] - y.loc[i, x.columns]).abs().max().max()))
        查(f"存档重放复现正典仓位(最大差 {差:.1e})", 差 < 1e-12)
    except Exception as e:
        查("重放一致性可比对", False, f"{type(e).__name__}: {e}")

    rr = 跑("引擎.重算权重", "--方案", "等权", "--输出", "结果_等权")
    查("换权重方案跑通(等权)", rr.returncode == 0, rr.stderr[-300:])
