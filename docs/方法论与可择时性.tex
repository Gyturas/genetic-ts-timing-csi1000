...
\section{一个可证伪的判据}
我们想回答的问题是:\emph{什么样的资产,值得做日频多空择时?}
做法是把``可择时''翻译成收益序列的三项可检验的统计性质。对每个资产的日频对数收益
$r_t$ 估计
\begin{equation}
...
\end{equation}
其中 $\tilde\sigma_{t-1}$ 为标准化条件波动(状态变量),$\tilde m_{t-1}$ 为过去 244 日
累计收益的标准化值(长期动量)。三个关键系数各对应一句白话:
\begin{itemize}
\item $\varphi_1>0$:\textbf{日频动量}——昨天涨,今天倾向继续涨。这是趋势类信号赖以
生存的原料;
\item $\varphi_2>0$:\textbf{波动溢价}——波动放大之后,次日期望收益为正(``恐慌之后有
反弹''),这是波动类信号的原料;
\item $\mu$(以买持夏普衡量):\textbf{漂移}——资产自身的长期趋势。它不是择时的
原料,反而是择时的\emph{税}(见下文)。
\end{itemize}
据此提出\textbf{可择时判据}——三关全过才算可择时:
\begin{quote}
(T1)~动量关:$\varphi_1>0$ 且 $t>2$,且 $\mathrm{VR}(5)>1.05$(动量能积累到多日,
不是隔日就抵消);\\
(T2)~波动溢价关:$\varphi_2>0$ 且 $t>1.5$;\\
(T3)~低漂移关:买入持有夏普 $|SR|<0.3$(择时不必缴``漂移税'')。
\end{quote}

\section{跨资产对比:四类资产,四种命运}
表~\ref{tab:cross} 摘录代表性资产(全表 23 个,2010 起日频)。读表方法:先看
$\varphi_1$、$\varphi_2$ 两列有没有星号(有结构吗?方向对吗?),再看买持夏普
(漂移税重不重?),最后对照末列——\emph{判据的预测与实盘表现逐一吻合}。四种命运:
\begin{center}\small
\begin{tabular}{ll}
\textbf{①~结构齐备}(中证1000、中证500) & 动量、波动溢价双显著 $+$ 低漂移 $\Rightarrow$ 可择时 \\
\textbf{②~什么都没有}(沪深300、黄金) & 各项系数均不显著 $\Rightarrow$ 无原料可挖 \\
\textbf{③~只有漂移}(黄金、纳斯达克) & 买持夏普高但无结构 $\Rightarrow$ 收益属于``拿住不动'' \\
\textbf{④~方向反了}(螺纹钢、铜) & $\varphi_1$ 显著为\emph{负}(日频反转)$\Rightarrow$ 吃不到嘴 \\
\end{tabular}
\end{center}

\begin{table}[htbp]\centering
\caption{代表性资产的三项判据}\label{tab:cross}
\footnotesize\setlength{\tabcolsep}{4.5pt}\begin{threeparttable}
\begin{tabular}{lcccccc}
\toprule
资产 & $\varphi_1$(短动量) & $\varphi_2$(bp,波动溢价) & VR(5) & 买持夏普 & 判据 & 实测 \\
\midrule
\textbf{中证1000} & $+0.055^{***}$ & $+11.0^{***}$ & $1.15$ & $0.20$ & \textbf{三关全过} & 爆炸 \\
                 & $(2.70)$        & $(2.84)$       &        &        &                  & \\[1pt]
中证500          & $+0.038^{*}$    & $+6.0^{*}$     & $1.10$ & $0.13$ & 全过(弱一档)   & 中等 \\
                 & $(1.95)$        & $(1.81)$       &        &        &                  & \\[1pt]
沪深300          & $+0.015$        & $-1.4$         & $1.02$ & $0.07$ & T1/T2 双败      & 弱 \\
                 & $(0.84)$        & $(-0.52)$      &        &        &                  & \\[1pt]
黄金现货          & $+0.016$        & $+2.1$         & $0.93$ & $0.84$ & 四项皆不显著      & 一般(纯运气) \\
                 & $(0.62)$        & $(0.46)$       &        &        &                  & \\[1pt]
纳斯达克100       & $-0.034$        & $+5.4$         & $0.83$ & $0.76$ & T1 负$+$高漂移   & 一般 \\
                 & $(-1.58)$       & $(1.62)$       &        &        &                  & \\[1pt]
螺纹钢            & $-0.046^{**}$   & $+2.2$         & $0.94$ & $-0.10$& T1 显著为负(反转)& 垃圾 \\
                 & $(-2.48)$       & $(0.82)$       &        &        &                  & \\[1pt]
铜               & $-0.044^{**}$   & $+1.4$         & $0.95$ & $0.18$ & T1 负$+\varphi_3$ 负 & 垃圾 \\
                 & $(-2.53)$       & $(0.69)$       &        &        &                  & \\
\bottomrule
\end{tabular}
\begin{tablenotes}\scriptsize
\item 括号内为 Bollerslev--Wooldridge 稳健 $t$ 值;$^{***}$、$^{**}$、$^{*}$ 分别表示
1\%、5\%、10\% 显著水平(双侧)。
\item ``实测''为本项目遗传规划择时在该资产上的表现(华泰复现或 GA 实挖)。判据与实测
排序完全一致;$\varphi_1$ 与实测绩效的 Spearman 秩相关 $+0.77$($p=0.004$)。
\end{tablenotes}\end{threeparttable}
\end{table}

\paragraph{(一)短期动量与波动溢价——中证1000 的两口肉。}
中证1000 是全样本\emph{唯一}$\varphi_1$、$\varphi_2$ 双双显著为正的资产。两个系数各翻译
一句:$\varphi_1=+0.055$——昨日每涨 1\%,今日期望多涨 5.5bp;$\varphi_2=+11$bp——波动
每高一个标准差,次日期望收益多 11bp。经济机制:前者来自小盘散户市场的\emph{涨跌延续与
龙头—跟风股的 lead--lag 传导};后者来自\emph{恐慌性抛售后的快速均值回归}(实测:高波动
日次日平均 $+0.25\%$,低波动日 $-0.15\%$)。两者都是小盘、散户化、高换手市场的结构性
特征,因此 A 股宽基呈单调梯度:中证1000 $>$ 中证500 $>$ 沪深300——市值越小、散户占比
越高,越可择时。
...
$F_{t,i}=f(\cdot)$——即``公式化因子''(formulaic alpha),其经典范式见
Kakushadze (2016) 的 \emph{101 Formulaic Alphas}。理想的 $f$ 使 $F_{t,i}$ 与次日收益 $r_{t+1,i}$ 稳定相关。挖因子即在
公式空间 $\mathcal F$ 上求 $\max_{f}\mathrm{IC}(f)$。$\mathcal F$ 大到无法穷举(公式可任意
嵌套),故用\textbf{遗传规划}(Koza, 1992)做定向随机搜索。公式化因子的自动生成
另有强化学习路线——以生成器直接最大化组合层面的协同性(AlphaGen,Yu et al.,
KDD 2023);本项目取 GP 路线,因其在小样本时序场景下每个个体都是低自由度的
强结构先验,过拟合面更小。
...
仅 $0.84$),rank 挖 + rank 权则在加权端丢信息。定稿采用``rank 挖 + cos 权''。``挖掘与组合分离、组合权重随时间动态刷新''
的架构亦见 AlphaForge(Shi et al., 2024);本项目以逐季 trailing cos IC 再加权
实现同一精神,并以其分子即因子自身盈亏的性质对齐目标函数。
...
\section{同方法跨指数落地:择时 vs 持有基准}
把\emph{完全相同}的挖矿引擎与 v4 定稿口径搬到 6 个宽基指数上(每指数用``其专属库
$\cup$ 类库''合成信号,执行对应 ETF,仅量价多空口径)。这是可择时性判据最直接的样本外
验证——\textbf{策略绩效的排序精确复刻了判据预测的排序},且与买入持有基准的对比揭示了
每个资产的性质。

\begin{table}[htbp]\centering
\caption{同一方法跨 6 指数回测(2018--2026,仅量价多空)与持有基准对比}\label{tab:cross_bt}
\small\begin{threeparttable}
\begin{tabular}{lc c cccc c cc}
\toprule
& & 因子 & \multicolumn{4}{c}{择时策略} & & \multicolumn{2}{c}{买入持有} \\
\cmidrule(lr){4-7}\cmidrule(lr){9-10}
指数 & ETF & 数 & 年化 & 夏普 & 回撤 & 卡玛 & & 年化 & 夏普 \\
\midrule
\textbf{中证1000} & 512100 & 241 & $\mathbf{27.4\%}$ & $\mathbf{1.35}$ & $-17.9\%$ & $\mathbf{1.53}$ & & $4.1\%$ & $0.19$ \\
中证500          & 510500 & 231 & $16.7\%$ & $0.92$ & $-15.7\%$ & $1.06$ & & $4.6\%$ & $0.20$ \\
科创50           & 588000 & 93  & $7.7\%$  & $0.36$ & $-33.9\%$ & $0.23$ & & $5.1\%$ & $0.24$ \\
中证红利          & 510880 & 271 & $3.7\%$  & $0.16$ & $-16.8\%$ & $0.22$ & & $4.9\%$ & $0.22$ \\
沪深300          & 510300 & 226 & $2.4\%$  & $0.06$ & $-34.6\%$ & $0.07$ & & $3.7\%$ & $0.16$ \\
创业板指          & 159915 & 205 & $1.5\%$  & $0.05$ & $-49.9\%$ & $0.03$ & & $10.1\%$& $0.39$ \\
\bottomrule
\end{tabular}
\begin{tablenotes}\scriptsize
\item 策略夏普梯度 中证1000 $\gg$ 中证500 $\gg$ 沪深300,与判据 $\varphi_1$ 梯度
($0.055>0.038>0.015$)一致——市值越小、越散户化,越可择时。
\item \textbf{沪深300}(夏普 0.06):判据 $\varphi_1$ 不显著、$\varphi_2$ 为负,双败;
实盘几乎做不出超额。\textbf{创业板指}(1.5\% vs 持有 10.1\%):买持夏普 0.39 为全场
最高漂移,正撞``大漂移''陷阱,择时把趋势收益做没了——``漂移税''的活教材。
\item 因子库同为一次挖矿产物;创业板指等样本内均挖出 200+ 因子却样本外失效,再证
``有因子 $\ne$ 可择时''——可择时性在资产,不在算法。
\end{tablenotes}\end{threeparttable}
\end{table}
...
Moreira \& Muir (2017); Cederburg et al.\ (2020). \quad
Yu, S.\ et al.\ (2023) Generating Synergistic Formulaic Alpha Collections via
Reinforcement Learning (AlphaGen), \emph{KDD}. \quad
Shi, H.\ et al.\ (2024) AlphaForge: A Framework to Mine and Dynamically Combine
Formulaic Alpha Factors, arXiv:2406.18394.
